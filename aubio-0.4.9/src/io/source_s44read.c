...
#include "fmat.h"
#include "ioutils.h"
#include "source_s44read.h"

#define AUBIO_S44READ_BUFSIZE 1024

//#define SHORT_TO_FLOAT(x) (smpl_t)(x * 3.0517578125e-05)

struct _aubio_source_s44read_t {
  uint_t hop_size;
  uint_t samplerate;
  uint_t channels;
...
aubio_source_s44read_t * new_aubio_source_s44read(const char_t * path, uint_t samplerate, uint_t hop_size) {
  ...
  if (path == NULL) {
    AUBIO_ERR("source_s44read: Aborted opening null path\n");
    goto beach;
  }
  if ((sint_t)samplerate < 0) {
    AUBIO_ERR("source_s44read: Can not open %s with samplerate %d\n", path, samplerate);
    goto beach;
  }
  if ((sint_t)hop_size <= 0) {
    AUBIO_ERR("source_s44read: Can not open %s with hop_size %d\n", path, hop_size);
    goto beach;
  }

//  s->path = AUBIO_ARRAY(char_t, strnlen(path, PATH_MAX) + 1);
//  strncpy(s->path, path, strnlen(path, PATH_MAX) + 1);
  s->path = AUBIO_ARRAY(char_t, strlen(path) + 1);
  strncpy(s->path, path, strlen(path) + 1);
  ...
  if (!s->fid) {
    AUBIO_STRERR("source_s44read: Failed opening %s (%s)\n", s->path, errorstr);
    goto beach;
  }
...
//  channels = read_little_endian(buf, 2);
  uint_t path_len = strlen(path);
  uint_t c = path[ path_len - 3 ];
  channels = (c == 's' || c == 'S') ? 2 : 
             (c == 'm' || c == 'M') ? 1 : 0;
  ...
  sr = memcmp(path + path_len - 2, "32", 2) == 0 ? 32000 :
       memcmp(path + path_len - 2, "44", 2) == 0 ? 44100 :
       memcmp(path + path_len - 2, "48", 2) == 0 ? 48000 : 0;

  // ByteRate
  //bytes_read += fread(buf, 1, 4, s->fid);
  //byterate = read_little_endian(buf, 4);
  byterate = sr * 2 * channels;
  ...
  if ( channels == 0 ) {
    AUBIO_ERR("source_s44read: Failed opening %s (number of channels can not be 0)\n", s->path);
    goto beach;
  }

  if ( (sint_t)sr <= 0 ) {
    AUBIO_ERR("source_s44read: Failed opening %s (samplerate can not be <= 0)\n", s->path);
    goto beach;
  }

  if ( byterate == 0 ) {
    AUBIO_ERR("source_s44read: Failed opening %s (byterate can not be 0)\n", s->path);
    goto beach;
  }

  if ( bitspersample == 0 ) {
    AUBIO_ERR("source_s44read: Failed opening %s (bitspersample can not be 0)\n", s->path);
    goto beach;
  }
#if 0
  if ( bitspersample != 16 ) {
  ...
  if ( byterate * 8 != sr * channels * bitspersample ) {
    AUBIO_ERR("source_s44read: Failed opening %s (wrong byterate)\n", s->path);
    goto beach;
  }

  if ( blockalign * 8 != channels * bitspersample ) {
    AUBIO_ERR("source_s44read: Failed opening %s (wrong blockalign)\n", s->path);
    goto beach;
  }
  ...
  if (samplerate == 0) {
    ...
    AUBIO_ERR("source_s44read: can not resample %s from %d to %dHz\n",
...
//  }
  ...
  s->output = new_fmat(s->input_channels, AUBIO_S44READ_BUFSIZE);
...
//  s->short_output = (unsigned char *)calloc(s->blockalign, AUBIO_WAVREAD_BUFSIZE);
  s->short_output = (unsigned char *)MALLOC(s->blockalign * AUBIO_S44READ_BUFSIZE);
  s->read_index = 0;
  s->read_samples = 0;
  s->eof = 0;
  ...
  for (j = 0; j < read; j++) {
    for (i = 0; i < s->input_channels; i++) {
      ...
      for (b = 0; b < bitspersample; b+=8 ) {
        ...
        unsigned_val = (unsigned_val << 8) + *(short_ptr);      // big endian
        short_ptr++;
      }
...
void aubio_source_s44read_do(aubio_source_s44read_t * s, fvec_t * read_data, uint_t * read){
  uint_t i, j;
  uint_t end = 0;
  uint_t total_wrote = 0;
  uint_t length = aubio_source_validate_input_length("source_s44read", s->path,
      s->hop_size, read_data->length);
  if (s->fid == NULL) {
    AUBIO_ERR("source_s44read: could not read from %s (file not opened)\n",
        s->path);
    return;
  }
  while (total_wrote < length) {
    ...
    if (total_wrote < length) {
      uint_t s44read_read = 0;
      aubio_source_s44read_readframe(s, &s44read_read);
      s->read_samples = s44read_read;
      s->read_index = 0;
      if (s->eof) {
        break;
      }
...
void aubio_source_s44read_do_multi(aubio_source_s44read_t * s, fmat_t * read_data, uint_t * read){
  uint_t i,j;
  uint_t end = 0;
  uint_t total_wrote = 0;
  uint_t length = aubio_source_validate_input_length("source_s44read", s->path,
      s->hop_size, read_data->length);
  uint_t channels = aubio_source_validate_input_channels("source_s44read",
...
uint_t aubio_source_s44read_seek (aubio_source_s44read_t * s, uint_t pos) {
  uint_t ret = 0;
  if (s->fid == NULL) {
    AUBIO_ERR("source_s44read: could not seek %s (file not opened?)\n", s->path, pos);
    return AUBIO_FAIL;
  }
  if ((sint_t)pos < 0) {
    AUBIO_ERR("source_s44read: could not seek %s at %d (seeking position should be >= 0)\n", s->path, pos);
    return AUBIO_FAIL;
  }
  ret = fseek(s->fid, s->seek_start + pos * s->blockalign, SEEK_SET);
  if (ret != 0) {
    AUBIO_STRERR("source_s44read: could not seek %s at %d (%s)\n", s->path, pos, errorstr);
    return AUBIO_FAIL;
  }
  // reset some values
  s->eof = 0;
...
uint_t aubio_source_s44read_close (aubio_source_s44read_t * s) {
  ...
  if (fclose(s->fid)) {
    AUBIO_STRERR("source_s44read: could not close %s (%s)\n", s->path, errorstr);
    return AUBIO_FAIL;
  }
  s->fid = NULL;
  return AUBIO_OK;
